...
void Solution(List<Product> products, List<Car> cars, ref int car_num, ref int prod_num)
{
    ...
    if (products[prod_num].arrival >= cars[car_num].start && products[prod_num].arrival <= cars[car_num].end)
    {
        ...
        prod_num++;
        products[prod_num - 1].car_num = cars[car_num].m + 1;
    }
    else
    {
        if (products[prod_num].arrival > cars[car_num].end)
            car_num++;
        else if (products[prod_num].arrival < cars[car_num].start)
            prod_num++;
    }
}
